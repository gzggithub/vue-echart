<template>
    <div class="app-correct">
        <div class="ove-title">设备技术参数智能纠错明细--配网设备</div>
            ...
            <el-table-column label="技术参数" min-width="450px" align="center">
                <template slot-scope="params">
                    <el-table :data="getParams(params.row)">
                        ...
                        <el-table-column label="参考值记录占比" prop="maxValuePer" :formatter="formateNum" :show-overflow-tooltip="true">               
                        </el-table-column> 
                        <el-table-column label="参考值记录纯比" prop="purePer" :formatter="formateNum" :show-overflow-tooltip="true">               
                        </el-table-column>
                    </el-table>
                </template>
            </el-table-column>
        ...
        <el-table :data="correctDetails" border fit :row-style="tableRowStyle">
            ...
            <el-table-column label="变压器类型" prop="byqlx" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <span class="error" v-if="errorbyqlx(scope.row.byqlx)">{{scope.row.byqlx}}</span>
                </template>    
            </el-table-column>
            <el-table-column label="包封方式" prop="bffs" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <span class="error" v-if="errorbffs(scope.row.bffs)">{{scope.row.bffs}}</span>
                </template> 
            </el-table-column>
            <el-table-column label="绕组导线材质" prop="rzdxcz" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <span class="error" v-if="errorrzdxcz(scope.row.rzdxcz)">{{scope.row.rzdxcz}}</span>
                </template>                 
            </el-table-column>
            <el-table-column label="铁芯型式"  prop="txxs" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <span class="error" v-if="errortxxs(scope.row.txxs)">{{scope.row.txxs}}</span>
                </template>                 
            </el-table-column>   
            <el-table-column label="油箱密封型式" prop="yxmfxs" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <span class="error" v-if="erroryxmfxs(scope.row.yxmfxs)">{{scope.row.yxmfxs}}</span>
                </template>                 
            </el-table-column>
            ...
            <el-table-column label="额定容量" prop="eddr" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <span class="error" v-if="erroreddr(scope.row.eddr)">{{scope.row.eddr}}</span>
                </template>    
            </el-table-column>
            <el-table-column label="分接范围" prop="fjfw" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <span class="error" v-if="errorfjfw(scope.row.fjfw)">{{scope.row.fjfw}}</span>
                </template> 
            </el-table-column>
            <el-table-column label="结线组别" prop="jxzb" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <span class="error" v-if="errorjxzb(scope.row.jxzb)">{{scope.row.jxzb}}</span>
                </template>                 
            </el-table-column>
            <el-table-column label="运行分接档位"  prop="yxfjdw" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <span class="error" v-if="erroryxfjdw(scope.row.yxfjdw)">{{scope.row.yxfjdw}}</span>
                </template>                 
            </el-table-column>   
            <el-table-column label="相数" prop="xs" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <span class="error" v-if="errorxs(scope.row.xs)">{{scope.row.xs}}</span>
                </template>                 
            </el-table-column>
            <el-table-column label="调压方式"  prop="tyfs" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <span class="error" v-if="errortyfs(scope.row.tyfs)">{{scope.row.tyfs}}</span>
                </template>                 
            </el-table-column>                                                 
        </el-table>
...
<script>
    import {queryClassifyNameAndTotal,queryDeviceModelAndTotal,queryManufacturerAndTotal,findOneViceDeviceAnswerByClassifyDeviceModeManufacture,queryViceDeviceAnswerByClassifyDeviceModeManufacture,queryVicedeviceCorrectDetails} from  "../../request/api.js"
    import equipCorrectChart from "@/components/echarts/equipCorrectChart"
    export default {
        name: "equipCorrectDetailVice",
        data() {
            return {
                type:'vice',
                classifys:[],
        ...
        methods: {
            ...
            modelChange(){
                ...
                this.pageNum = 1
                this.pageSize = 10
                findOneViceDeviceAnswerByClassifyDeviceModeManufacture({
                    'classifyName':this.selectClassify,
                    'manufacturer':this.selectManufacturer,
            ...
            queryErrors(){
                queryVicedeviceCorrectDetails({
                    'classifyName':this.selectClassify,
                    'manufacturer':this.selectManufacturer,
                    'deviceModel':this.selectModel,
                    'byqlx':this.correctValue.byqlx,
                    'bffs':this.correctValue.bffs,
                    'rzdxcz':this.correctValue.rzdxcz,
                    'txxs':this.correctValue.txxs,
                    'yxmfxs':this.correctValue.yxmfxs,
                    'eddy':this.correctValue.eddy,  
                    'eddr':this.correctValue.eddr,   
                    'fjfw':this.correctValue.fjfw,
                    'jxzb':this.correctValue.jxzb,
                    'yxfjdw':this.correctValue.yxfjdw,
                    'xs':this.correctValue.xs,  
                    'tyfs':this.correctValue.tyfs,                                                                                              
            ...
            errorbyqlx(byqlx){
                return byqlx != this.correctValue.byqlx
            },
            errorbffs(bffs){
                return bffs != this.correctValue.bffs
            },
            errorrzdxcz(rzdxcz){
                return rzdxcz != this.correctValue.rzdxcz
            },
            errortxxs(txxs){
                return txxs != this.correctValue.txxs
            },
            erroryxmfxs(yxmfxs){
                return yxmfxs != this.correctValue.yxmfxs
            },
            erroreddy(eddy){
                return eddy != this.correctValue.eddy
            },            
            erroreddr(eddr){
                return eddr != this.correctValue.eddr
            },  
            errorfjfw(fjfw){
                return fjfw != this.correctValue.fjfw
            },
            errorjxzb(jxzb){
                return jxzb != this.correctValue.jxzb
            },

            erroryxfjdw(yxfjdw){
                return yxfjdw != this.correctValue.yxfjdw
            },
            errorxs(xs){
                return xs != this.correctValue.xs
            },
            errortyfs(tyfs){
                return tyfs != this.correctValue.tyfs
            },  
            getParams(row){
