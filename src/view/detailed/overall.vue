<template>
    <div class="app-problem">
        <div class="problem-left">
            <left-list @child-click="childClick" v-bind:ind=this.$route.params.ind></left-list>
        </div>
        ...
        <div class="problem-right ove-right-pad">
            ...
            <div class="tab-out">
                <el-table v-if=showTableAll :data="tableDataAll" :span-method="objectSpanMethod"
                    style="margin-bottom:20px" border stripe size="small">
                    ...
                    <el-table-column label="完整性" align="center">
                        <template slot-scope="scope">
                            <span class="tb-span" @click="cellClick(scope.row, 1)">{{scope.row.fullName}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="规范性" align="center">
                        <template slot-scope="scope">
                            <span class="tb-span" @click="cellClick(scope.row, 2)">{{scope.row.normName}}</span>
                        </template>
                    </el-table-column>
            ...
            <el-table :data="tableDataTwo" border stripe size="small">
            ...
            <div v-if="tableDataTwo.length > 0" class="ove-paging">
                <el-pagination v-if="pageshow" background @size-change="handleSizeChange"
                    @current-change="handleCurrentChange" :page-sizes="[20, 50, 100]" :page-size="20"
    ...
    export default {
        ...
        methods: {
            ...
            exportData() {
                ...
                current=${this.current}&errorType=${this.errorType}&flag=${this.flag}&professional=${this.professional}&size=${this.size}&status=${this.status}&id=${this.changeId}&professionLevel=${this.professionLevel}`
                location.href = url;
            },
        ...
        mounted() {
            ...
            /*if (this.$route.params.orgId) {
                this.subDept = this.$route.params.orgId.toString();
                this.getMajor();
            }*/
            //如果传入的专业存在
